        ...
        <div className="h-full grid grid-cols-2 rounded-lg p-5 flex-1 bg-gray-200 border-2 border-zinc-300 shadow-2xl">
          <div className="flex justify-center break-after-right">
            <img
              src={productId.image}
              alt={productId.name}
              className="flex w-full h-96 object-cover rounded shadow-lg"
            />
          </div>
          <div className="flex h-96 align-items-left flex-col justify-center pl-5">
            <h1 className="text-3xl font-extrabold text-gray-900 sm:text-5xl underline pb-2">
              {productId.name}
            </h1>
            <div className="border border-gray-300 p-4 rounded-md mt-4 text-gray-500">
              {productId.summary}
            </div>
            <div className="text-gray-500 mt-1 mb-5">
              Stock: {productId.stock}
            </div>
            <div className="text-black font-bold py-2 px-4 rounded-md border border-green-600">
              Precio: ${productId.price}
            </div>
            <button
              onClick={() => agregarAlCarro()}
              className="bg-green-500 text-white font-bold py-2 px-4 rounded-md border border-green-600 mt-5"
            >
              Agregar al carro
            </button>
          </div>
