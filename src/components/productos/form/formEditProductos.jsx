 ...
 async function handleSubmit(e){
    ...
    if(!Object.keys(errors).at(0)){
        ...
        await axios.put(`/product/${productId}/admin/${idAdmin}`,input).then((response)=> {
            alert('Producto actualizado con éxito')}).catch(err=>{return(console.log(err) ,alert(err))})        
          closeModal()
  ...
  return (
    <div className="p-4 shadow-lg text-gray-700 bg-gray-200 w-96 m-auto min-h-fit dark:text-gray-900 dark:bg-gray-900 ">
      ...
      <form className="flex flex-col "onSubmit={(e)=> handleSubmit(e) }>
        ...
        <div className="flex flex-col justify-center ">
          <label className="mb-2 font-bold dark:text-gray-100 ">Precio: </label>
          <input className="border border-gray-400 bg-white h-10 px-5 pr-16 rounded-lg text-sm focus:outline-none"
           type="number"
           min="0"
            value={input.price}
        ...
        <div className="flex flex-col justify-center ">
          <label className="mb-2 font-bold dark:text-gray-100  ">Stock: </label>
          <input className="border border-gray-400 bg-white h-10 px-5 pr-16 rounded-lg text-sm focus:outline-none"
           type="number"
           min="0"
            value={input.stock}
        ...
        <div className="flex flex-col justify-center  ">
          <label className="mb-2 font-bold dark:text-gray-100  " >Detalle del Producto: </label>
        ...
        <div className="flex flex-col justify-center items-center w-full h-72 ">
          <label className="mb-2 font-bold dark:text-gray-100 ">Imagen del Producto</label>
          {input.image&&<img className="border border-gray-400 h-28 w-28 object-cover m-auto  bg-white  " src={input.image} alt='productImage'/>}
          ...
          /><label
          htmlFor="fileInput"
          className="inline-flex relative  mt-2 mb-6 h-14 w-14 items-center justify-center px-4 py-2 bg-teal-400 rounded-full font-medium hover:bg-teal-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500"
        >
  ...
  <line x1="18" y1="3" x2="18" y2="9" />
        ...
        <div className="dark:text-gray-100 ">
          Categoria :
          <select value={input.categoryId} className="dark:text-gray-900 " onChange={(e)=>setInput({...input, categoryId: e.target.value})}>
        ...
        <div className="flex flex-col">
          <label className="dark:text-gray-100 ">Descuento? : <input type="checkbox" checked={input.discount.active} name="discount.active" onChange={(e)=>setInput({...input, discount:{active:!input.discount.active, value:input.discount.value}})}></input></label>
          <label className="dark:text-gray-100 ">Porcentaje a descontar: <input className="w-12 dark:text-slate-800 ml-2 pl-2" type="number"  value={input.discount.value} onChange={(e)=>setInput({...input, discount:{active:input.discount.active, value:e.target.value}})}></input> </label>
        </div>
        <div>
          <label className="dark:text-gray-100 mx-4 ">
            Activo: 
            <input type="checkbox" checked={input.active} className="ml-3" onChange={(e)=>setInput({...input, active:!input.active})}></input>
          </label>
        </div>
        <div className="">
